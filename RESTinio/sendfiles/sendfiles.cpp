...
// original : https://github.com/Stiffstream/restinio/tree/master/dev/sample/sendfiles
...
auto server_handler( const std::string & root_dir )
{
	...
	router->http_get(
		...
		[ server_root_dir ]( auto req, auto params ){
			...
			std::string file_path = server_root_dir + "index.html";
...
// countermeasures against directory traversal
			if( std::string::npos == path.find( ".." ) )
			{
				...
				{
					std::string body = build_msg_not_found(str_path);
std::cout << body << std::endl;
 					std::string content_type("text/html");
					return send_not_found(req, body,  content_type);
				}
			}
			else
			{
...
void print_app_args( struct app_args_t args)
{
	...
	std::cout << "root_dir: " << args. m_root_dir << std::endl;
}
