...
// original : https://github.com/ohmae/ImageIO
// change log
// 2020-02-01
// C++ でコンパイルできるように修正
...
printf("bmp header: biBitCount = %d , biCompression= %d \n", header->info.biBitCount, header->info.biCompression);
...
static result_t write_header(
  ...
  if (bc <= 8) {
    ...
    header = (uint8_t *)calloc(header_size, 1);
    if (header  == NULL) {
        return FAILURE;
    }
...
static result_t write_palette(FILE *fp, image_t *img, int bc) {
  ...
  int palette_size = (1 << bc) * 4;
    ...
    buffer = (uint8_t *)calloc(palette_size, 1);
    if (buffer  == NULL) {
        return FAILURE;
    }
...
static result_t write_bitmap_rle(FILE *fp, image_t *img, int bc, int stride) {
  ...
  raw = (uint8_t *)calloc(stride, 1);
  step = (uint8_t *)calloc(stride, 1);
  row = (uint8_t *)calloc(stride * 2, 1);
