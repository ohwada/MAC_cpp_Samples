...
// original : https://github.com/PacktPublishing/The-Modern-Cpp-Challenge/blob/master/Chapter09/problem_77/main.cpp
...
#ifdef _WIN32
static const std::string font_path = R"(c\windows\fonts\Arial.ttf)";
#elif defined (__APPLE__)
static const std::string font_path = 
R"(/Library/Fonts/Arial Unicode.ttf)";
#else
static const std::string font_path = 
R"(/usr/share/fonts/Arial.ttf)";
#endif
   ...
   std::string_view path)
{
   ...
   auto font = pdf.GetFontForFile(font_path);
if(!font){
    std::cout << "cannot load font: " << font_path << std::endl;
    return;
}
...
const std::string FILE("movies.pdf");

   print_pdf(movies, FILE);
