...
enum nk_button_behavior {NK_BUTTON_DEFAULT, NK_BUTTON_REPEATER, NK_BUTTON_DEFAULT2};
    ...
    if (nk_input_is_mouse_hovering_rect(i, r)) {
        *state = NK_WIDGET_STATE_HOVERED;
        ...
        if (nk_input_has_mouse_click_in_rect(i, NK_BUTTON_LEFT, r)) {
            ret = (behavior != NK_BUTTON_DEFAULT && behavior != NK_BUTTON_DEFAULT2) ?
                nk_input_is_mouse_down(i, NK_BUTTON_LEFT):
#ifdef NK_BUTTON_TRIGGER_ON_RELEASE
                nk_input_is_mouse_released(i, NK_BUTTON_LEFT);
#else
                (behavior == NK_BUTTON_DEFAULT2) ? nk_input_is_mouse_released(i, NK_BUTTON_LEFT) :
