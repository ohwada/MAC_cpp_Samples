 ...
 * overwriteWavHeader
 */
bool overwriteWavHeader(char *filename, int channels, int samplingrate, int bits)
{
    ...
    bool ret = writeWavHeaderFp(fp, channels, samplingrate, bits, filesize);
	fclose(fp);
    return ret;
}
 ...
 * writeHeaderFp
 */
bool writeWavHeaderFp(FILE *fp, int channels, int samplingrate, int bits, int filesize)
{
	...
	wavh.wBitsPerSample = bits; // maybe 16
