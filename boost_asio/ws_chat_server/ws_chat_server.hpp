...
/**
 * class ws_chat_session
 */
class ws_chat_session
  : public chat_participant,
    public boost::enable_shared_from_this<ws_chat_session>
{
public:
/**
 *  ws_chat_session
 */
  ws_chat_session(boost::asio::io_context& io_context, chat_room& room)
    : socket_(io_context),
      room_(room)
  {
        m_id = 0;
 ...
 */
  void start(int id)
  {
    ...
    socket_.async_read_some(boost::asio::buffer(m_data, max_length),
        boost::bind(&ws_chat_session::handle_read_handshake, this,
  ...
  void deliver(const std::string& msg)
  {
    ...
    if (!write_in_progress)
    {
        ...
        if(size > 0 ){
            ...
            boost::bind(&ws_chat_session::handle_write_msgs, shared_from_this(),
            boost::asio::placeholders::error));
        }
    }
  }
      ...
      size_t bytes_transferred)
  {
    ...
    if (!error)
    {
        ...
        if(res == RES_HANDSHAKE) {
                ...
                boost::bind(&ws_chat_session::handle_write_handshake, this,
                boost::asio::placeholders::error));
        } else if(res == RES_BRAWSER) {
                boost::asio::async_write(socket_,
                boost::asio::buffer(write_data),
                boost::bind(&ws_chat_session::handle_write_brawser, this,
                boost::asio::placeholders::error));
        } // res
    }
    else
...
// wait next message
        socket_.async_read_some(boost::asio::buffer(m_data, max_length),
        boost::bind(&ws_chat_session::handle_read_frame, this,
        boost::asio::placeholders::error,
        boost::asio::placeholders::bytes_transferred));
    }
    else
    ...
    if (!error)
    {
        ...
        if(res == RES_TEXT) {
            ...
            if(write_data_size>0) {
                ...
                boost::bind(&ws_chat_session::handle_write_frame, this,
                boost::asio::placeholders::error));
            } else {
...
typedef boost::shared_ptr<ws_chat_session> ws_chat_session_ptr;


/**
 * class ws_chat_server
 */
class ws_chat_server
{
public:
  ws_chat_server(boost::asio::io_context& io_context,
  ...
  void start_accept()
  {
    ws_chat_session_ptr new_session(new ws_chat_session(io_context_, room_));
    acceptor_.async_accept(new_session->socket(),
        boost::bind(&ws_chat_server::handle_accept, this, new_session,
          boost::asio::placeholders::error));
  }

  void handle_accept(ws_chat_session_ptr session,
...
bool run_server(unsigned short port)
{
  ...
  try
  {
        ...
        ws_chat_server s(io_context, endpoint);
