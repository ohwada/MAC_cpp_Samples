//  g++ sample1.cpp `pkg-config --cflags --libs nss` 

// https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSS/nss_sample_code/NSS_Sample_Code_Sample1
...
Server::getPrivateKey(SECKEYPrivateKey **prvKey)
{
  ...
  std::cout << "getPrivateKey: list = " << list << std::endl;
...
main(int argc, char *argv[])
{
  ...
  // Initialize NSS
  rv = InitNSS();
  if (rv) { std::cout << "InitNSS failed" << std::endl; goto done; }

  // Create the first "server"
  server1 = new Server("Server1");
  if (!server1 || server1->Init())
  {
    std::cout << "Server1 could not be created" << std::endl;
    rv = 1;
    goto done;
  }
  ...
  if (rv) { std::cout << "GenerateKeys failed" << std::endl; goto done; }

  // Now that everything is ready, start server1. This loads
  // the encryption and MAC keys from the "files"
  rv = server1->Start();
  if (rv) { std::cout << "Cannot start server 1" << std::endl; goto done; }
  ...
  {
    std::cout << "Server2 could not be created" << std::endl;
    rv = 1; // Error
    goto done;
  }
  ...
  // Transfer the keys from server1
  {
    ...
    if (rv) { std::cout << "ExportPublicKey failed" << std::endl; goto trans_done; }

    // Send the public key to server 1 and get back the
    // wrapped key values
    rv = server1->ExportKeys(pubKeyData, &wrappedEncKey, &wrappedMacKey);
    if (rv) { std::cout << "ExportKeys failed" << std::endl; goto trans_done; }

    // Print - for information
    std::cout << "Wrapped Encryption Key" << std::endl;
    printBuffer(wrappedEncKey->data, wrappedEncKey->len);
    std::cout << "Wrapped MAC Key" << std::endl;
    ...
    if (rv) { std::cout << "ImportKeys failed" << std::endl; goto trans_done; }
  ...
  rv = server2->Start();
  if (rv) { std::cout << "Cannot start server 2" << std::endl; goto done; }
  ...
  // List keys in the token - informational
  {
    ...
    std::cout << "List Private Keys" << std::endl;
  ...
  if (rv) { std::cout << "Key Comparison failed" << std::endl; }
