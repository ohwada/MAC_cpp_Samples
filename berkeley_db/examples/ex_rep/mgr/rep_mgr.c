// changlog
// 2021-02-01 K.OHWADA
// set DB_REPMGR_CONF_DISABLE_SSL


// ex_rep
// Replicated stock quote server // Berkeley DB supports building highly available applications via replication groups, which contain a master environment and one or more read-only clients. Replicas may be on the same machine or connected by local or wide-area networks.
// The replication example is a small stock quote server. 
// There are two versions of the program: 
// one uses Berkeley DB's Replication Manager support, 
// and the other uses the underlying base replication API.
...
// build code
// % gcc rep_mgr.c  `pkg-config --cflags --libs libdb`

// run code
// % ./a.out  -M -h server -l localhost:6000 



// https://github.com/berkeleydb/libdb/blob/master/examples/c/ex_rep/mgr/rep_mgr.c
...
// 2021-02-01
	dbenv-> rep_set_config( dbenv, DB_REPMGR_CONF_DISABLE_SSL, 1 );
