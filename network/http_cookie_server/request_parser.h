...
// prototype
bool parse_path(char* text,  char* path);
bool parse_cookie_visits(char* request,  int *visits);
 ...
 *  parse_request
 */
bool parse_cookie_visits(char* text,  int *visits)
{
    const char NUL = '\0';

    const char PATTERN[] =  "Cookie:.*visits=([0-9]+)";
  const size_t MATCH_SIZE = 4;
const size_t BUFSIZE = 100;
