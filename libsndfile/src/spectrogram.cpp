...
// reference : http://www.mega-nerd.com/libsndfile/tools/#spectrogram
 ...
 * render_spectrogram
 */
static void render_spectrogram (cairo_surface_t * surface, double spec_floor_db, float **mag2d, double maxval, double left, double top, double width, double height, bool gray_scale)
{
 ...
 * render_spect_border
 */
static void render_spect_border (cairo_surface_t * surface, const char * filename, double left, double width, double seconds, double top, double height, double min_freq, double max_freq, bool log_freq)
{
	char text [512] ;
	cairo_t * cr ;
...
/**
 * magindex_to_specindex
 */
static double magindex_to_specindex (int speclen, int maglen, int magindex, double min_freq, double max_freq, int samplerate, bool log_freq)
{
...
static void interp_spec (float * mag, int maglen, const double *spec, int speclen, const RENDER *render, int samplerate)
{
	...
	*/
	...
	{	/* Average the pixels in the range it comes from */
		double this1 = magindex_to_specindex (speclen, maglen, k,
		...
		{	/* The output indices are more sparse than the input indices
			...
			*/
			...
			double count = 1.0 - (this1 - floor (this1)) ;
			double sum = spec [(int) this1] * count ;

			while ((this1 += 1.0) < next && (int) this1 <= speclen)
			{	sum += spec [(int) this1] ;
				count += 1.0 ;
				}
		...
		*/
		{	/* Take a weighted average of the nearest values */
			mag [k] = spec [(int) this1] * (1.0 - (this1 - floor (this1)))
						+ spec [(int) this1 + 1] * (this1 - floor (this1)) ;
			} ;
		} ;
 ...
 * render_to_surface
 */
static void render_to_surface (const RENDER * render, SNDFILE *infile, int samplerate, sf_count_t filelen, cairo_surface_t * surface)
{
	...
	*/
	...
	mag_spec = (float **)calloc (width, sizeof (float *)) ;
	if (mag_spec == NULL)
	...
	for (w = 0 ; w < width ; w++) {	
		float *mag_spec_w = (float *)calloc (height, sizeof (float));
		if(!mag_spec_w){
			printf ("%s : Not enough memory.\n", __func__) ;
			exit (1) ;
		}
		mag_spec[w] = mag_spec_w;
	} // for
 ...
 */
static void render_cairo_surface (const RENDER * render, SNDFILE *infile, int samplerate, sf_count_t filelen)
{
...
int main(int argc, char **argv) 
{
	...
	string fname = getFileNameWithoutExt(render.sndfilepath);
	string outfname = "spectrogram_" + fname  + ".png";
	render.pngfilepath = outfname.c_str();
	...
	cout << "created: " << render.pngfilepath << endl;
	return EXIT_SUCCESS;
} // main
